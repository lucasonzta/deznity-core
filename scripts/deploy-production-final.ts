...
/**
 * Ejecuta una tarea de deployment usando OpenRouter
 */
...
Genera una respuesta detallada y específica para esta tarea de deployment en producción.`;
    ...
    // Llamar a OpenRouter con diferentes modelos según el agente
    let model = 'openai/gpt-4o'; // Modelo por defecto
    
    if (task.agent.includes('UX')) {
      model = 'anthropic/claude-3.5-sonnet';
    } else if (task.agent.includes('Finance') || task.agent.includes('Sales')) {
      model = 'openai/gpt-4o';
...
async function deployProduction(): Promise<void> {
  ...
  try {
    ...
    console.log('✅ OpenRouter configurado y funcionando');
